...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-white relative overflow-x-hidden">
      {/* Background organic shapes — sized to viewport so they never widen the page */}
      <div className="absolute -top-[10%] -left-[20%] w-[min(800px,120vw)] h-[min(800px,120vw)] max-w-none opacity-50 pointer-events-none">
      ...
      <div className="absolute -bottom-[10%] -right-[15%] w-[min(900px,130vw)] h-[min(900px,130vw)] opacity-40 pointer-events-none">
        <svg viewBox="0 0 900 900" className="w-full h-full" preserveAspectRatio="xMidYMid slice">
          <path
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[min(500px,90vw)] h-[min(500px,90vw)] opacity-30 pointer-events-none">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/70 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 sm:h-20 gap-2 min-w-0">
            <Link
              to="/"
              className="text-2xl sm:text-3xl font-black text-[#2c3e73] tracking-tight hover:text-[#1ac8db] transition shrink-0"
            >
              IS
            </Link>
            ...
            <Link to="/" className="min-w-0">
              <Button
                variant="ghost"
                className="text-[#2c3e73] hover:text-[#1ac8db] gap-1 sm:gap-2 text-sm sm:text-lg font-medium px-2 sm:px-4"
              >
                <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
                <span className="truncate sm:whitespace-normal">Back to Home</span>
              </Button>
            </Link>
          </div>
        </div>
      ...
      <main className="min-h-screen flex items-center justify-center pt-24 pb-16 px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-lg w-full">
          <div className="text-center mb-10 space-y-4">
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-black text-[#2c3e73] tracking-tight leading-tight">
              Login
            </h1>
